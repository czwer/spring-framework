...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public <T> T createBean(Class<T> beanClass) throws BeansException {
		...
		logger.info("[SPRING] 自定义日志---createBean中，调用createBean方法："+beanClass.getName());
	...
	public Object configureBean(Object existingBean, String beanName) throws BeansException {
		...
		logger.info("[SPRING] 自定义日志---configureBean中，调用initializeBean方法："+beanName);
	...
	public Object initializeBean(Object existingBean, String beanName) {
		logger.info("[SPRING] 自定义日志---initializeBean中，调用initializeBean方法："+beanName);
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		beanPostProcessors.forEach(b->{			logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsBeforeInitialization】存在的BeanPostProcessor："+b.getClass().getName()+",beanName："+beanName);});
		Object result = existingBean;
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			Object current = processor.postProcessBeforeInitialization(result, beanName);
			if (current == null) {
				logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsBeforeInitialization】调用所有BeanPostProcessor的postProcessBeforeInitialization方法，postProcessBeforeInitialization返回Null："+beanName+",BeanPostProcessor："+processor.getClass().getName()+"，返回bean实例："+result.getClass().getName());
				return result;
			}
			logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsBeforeInitialization】调用所有BeanPostProcessor的postProcessBeforeInitialization方法，postProcessBeforeInitialization返回非Null，后续BeanPostProcessor继续处理："+beanName+",BeanPostProcessor："+processor.getClass().getName()+"，处理后的bean实例："+current.getClass().getName());
			result = current;
		}
		return result;
	}
	...
	public Object applyBeanPostProcessorsAfterInitialization(Object existingBean, String beanName)
		...
		Object result = existingBean;
		List<BeanPostProcessor> beanPostProcessors = getBeanPostProcessors();
		beanPostProcessors.forEach(b->{			logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsAfterInitialization】存在的BeanPostProcessor："+b.getClass().getName()+",beanName："+beanName);});
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			Object current = processor.postProcessAfterInitialization(result, beanName);
			if (current == null) {
				logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsAfterInitialization】调用BeanPostProcessor.postProcessAfterInitialization方法，postProcessAfterInitialization方法返回Null："+beanName+",BeanPostProcessor："+processor.getClass().getName()+"，返回bean实例："+result.getClass().getName());
				return result;
			}
			logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsAfterInitialization】调用所有BeanPostProcessor的postProcessAfterInitialization方法，postProcessAfterInitialization返回非Null，后续BeanPostProcessor继续处理："+beanName+",BeanPostProcessor："+processor.getClass().getName()+"，处理后的bean实例："+current.getClass().getName());
			result = current;
		}
		return result;
	}
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		logger.info("[SPRING] 自定义日志【关键流程-createBean】---"+beanName);
		...
		if (resolvedClass != null){
			logger.info("[SPRING] 自定义日志---【createBean】调用resolveBeanClass方法：处理Bean Class的解析与缓存，Bean Class:"+resolvedClass.getName()+"，beanName："+beanName);
		}else {
			logger.info("[SPRING] 自定义日志---【createBean】调用resolveBeanClass方法：处理Bean Class的解析与缓存，Bean Class null："+beanName);
		}
		if (resolvedClass != null && !mbd.hasBeanClass() && mbd.getBeanClassName() != null) {
			mbdToUse = new RootBeanDefinition(mbd);
			mbdToUse.setBeanClass(resolvedClass);
			try {
				logger.info("[SPRING] 自定义日志---【createBean】调用prepareMethodOverrides方法："+beanName);
		...
		try {
			...
			logger.info("[SPRING] 自定义日志---【createBean】【核心】调用resolveBeforeInstantiation方法："+beanName);
			Object bean = resolveBeforeInstantiation(beanName, mbdToUse);
			if (bean != null) {
				logger.info("[SPRING] 自定义日志---【createBean】【核心】调用resolveBeforeInstantiation方法【创建Bean的快捷方式，实例化前的短路机制】给BeanPostProcessor一个机会去创建代理对象来代替目标Bean，返回【代理】bean实例："+beanName);
				return bean;
			}
		}
		catch (Throwable ex) {
		...
		try {
			logger.info("[SPRING] 自定义日志---【createBean】调用doCreateBean方法："+beanName);
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
			throws BeanCreationException {
		logger.info("[SPRING] 自定义日志【关键流程-doCreateBean】---"+beanName);
		...
		if (instanceWrapper == null) {
			logger.info("[SPRING] 自定义日志---【doCreateBean】调用createBeanInstance方法创建instanceWrapper："+beanName);
			instanceWrapper = createBeanInstance(beanName, mbd, args);
		}else {
			logger.info("[SPRING] 自定义日志---【doCreateBean】已从factoryBeanInstanceCache中获取到instanceWrapper："+beanName);
		}
		...
		logger.info("[SPRING] 自定义日志---【doCreateBean】"+beanName+",getWrappedInstance返回："+bean.getClass().getName()+",getWrappedClass："+beanType.getName());

		// Allow post-processors to modify the merged bean definition.
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					logger.info("[SPRING] 自定义日志---【doCreateBean】调用applyMergedBeanDefinitionPostProcessors方法："+beanName);
		...
		if (earlySingletonExposure) {
			...
			logger.info("[SPRING] 自定义日志---【doCreateBean】调用getEarlyBeanReference方法 & 调用addSingletonFactory方法 ："+beanName);
		...
		try {
			logger.info("[SPRING] 自定义日志---【doCreateBean】调用populateBean方法：依赖注入"+beanName);
			populateBean(beanName, mbd, instanceWrapper);
			logger.info("[SPRING] 自定义日志---【doCreateBean】调用initializeBean方法：初始化Bean：调用Aware接口，调用BeanPostProcessor.postProcessBeforeInitialization，调用invokeInitMethods，调用BeanPostProcessor.postProcessAfterInitialization，"+beanName);
		...
		if (earlySingletonExposure) {
			logger.info("[SPRING] 自定义日志---【doCreateBean】doCreateBean步骤：5-0，doCreateBean中，调用getSingleton方法："+beanName);
			...
			if (earlySingletonReference != null) {
				if (exposedObject == bean) {
					...
					logger.info("[SPRING] 自定义日志---【doCreateBean】doCreateBean步骤：5-1，doCreateBean中，调用getDependentBeans方法："+beanName);
		...
		try {
			logger.info("[SPRING] 自定义日志---【doCreateBean】doCreateBean步骤：5-2，doCreateBean中，调用registerDisposableBeanIfNecessary方法："+beanName);
	...
	protected ResolvableType getTypeForFactoryBean(String beanName, RootBeanDefinition mbd, boolean allowInit) {
		...
		if (allowInit) {
			...
			if (factoryBean != null) {
				...
				logger.info("[SPRING] 自定义日志---getTypeForFactoryBean中，调用getTypeForFactoryBean方法：beanName："+beanName);
		...
		try {
			...
			try {
				...
				if (instance == null) {
					...
					logger.info("[SPRING] 自定义日志---factoryBeanInstanceCache中放入BeanWrapper："+beanName);
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					logger.info("[SPRING] 自定义日志---【resolveBeforeInstantiation】调用applyBeanPostProcessorsBeforeInstantiation方法："+beanName);
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					if (bean != null) {
						logger.info("[SPRING] 自定义日志【关键流程-resolveBeforeInstantiation】---"+beanName);
						logger.info("[SPRING] 自定义日志---调用applyBeanPostProcessorsAfterInitialization方法：给Bean设置属性等各种配置的过程："+beanName);
	...
	protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
			...
			if (result != null) {
				logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsBeforeInstantiation】调用InstantiationAwareBeanPostProcessor.postProcessBeforeInstantiation方法：从Class到对象创建的过程【代理入口一】,返回Bean实例："+beanName+",InstantiationAwareBeanPostProcessor："+bp.getClass().getName());
				return result;
			}
		}
		logger.info("[SPRING] 自定义日志---【applyBeanPostProcessorsBeforeInstantiation】调用InstantiationAwareBeanPostProcessor.postProcessBeforeInstantiation方法,返回Null："+beanName);
		return null;
	}
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		logger.info("[SPRING] 自定义日志【关键流程-createBeanInstance】---" + beanName);
		// Make sure bean class is actually resolved at this point.
		...
		if (args == null) {
			...
			if (instanceSupplier != null) {
				logger.info("[SPRING] 自定义日志---【createBeanInstance】Supplier回调方式创建bean实例：" + beanName);
				return obtainFromSupplier(instanceSupplier, beanName, mbd);
			}
		}

		if (mbd.getFactoryMethodName() != null) {
			logger.info("[SPRING] 自定义日志---【createBeanInstance】工厂方法创建bean实例：" + beanName);
		...
		if (resolved) {
			if (autowireNecessary) {
				logger.info("[SPRING] 自定义日志---【createBeanInstance】构造函数自动注入方式创建bean实例-1：" + beanName);
				return autowireConstructor(beanName, mbd, null, null);
			}
			else {
				logger.info("[SPRING] 自定义日志---【createBeanInstance】默认无参构造函数方式创建bean实例-1：" + beanName);
		...
		if (ctors != null || mbd.getResolvedAutowireMode() == AUTOWIRE_CONSTRUCTOR ||
			...
			logger.info("[SPRING] 自定义日志---【createBeanInstance】构造函数自动注入方式创建bean实例-2：" + beanName);
		...
		if (ctors != null) {
			logger.info("[SPRING] 自定义日志---【createBeanInstance】构造函数自动注入方式创建bean实例-3：" + beanName);
			return autowireConstructor(beanName, mbd, ctors, null);
		}

		// No special handling: simply use no-arg constructor.
		logger.info("[SPRING] 自定义日志---【createBeanInstance】默认无参构造函数方式创建bean实例-2：" + beanName);
		return instantiateBean(beanName, mbd);
	}
	...
	protected BeanWrapper instantiateBean(String beanName, RootBeanDefinition mbd) {
		try {
			logger.info("[SPRING] 自定义日志---【获取Bean】【创建bean实例】：instantiateBean中调用getInstantiationStrategy().instantiate(mbd, beanName, this)：" + beanName);
			Object beanInstance = getInstantiationStrategy().instantiate(mbd, beanName, this);
			BeanWrapper bw = new BeanWrapperImpl(beanInstance);
			initBeanWrapper(bw);
			logger.info("[SPRING] 自定义日志---【获取Bean】【创建bean实例】：instantiateBean返回BeanWrapper" + beanName);
			return bw;
		}
		catch (Throwable ex) {
			throw new BeanCreationException(mbd.getResourceDescription(), beanName, ex.getMessage(), ex);
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		if (bw == null) {
			if (mbd.hasPropertyValues()) {
				...
				logger.info("[SPRING] 自定义日志---【populateBean】跳过属性填充阶段：空实例："+beanName);
				return;
			}
		}
		...
		if (bw.getWrappedClass().isRecord()) {
			if (mbd.hasPropertyValues()) {
				...
				logger.info("[SPRING] 自定义日志---【populateBean】跳过属性填充阶段：它们是不可变的："+beanName);
				return;
			}
		}
	...
	protected void autowireByName(
		...
		for (String propertyName : propertyNames) {
			if (containsBean(propertyName)) {
				logger.info("[SPRING] 自定义日志---【autowireByName】beanName："+beanName+"正在依赖注入，调用getBean："+propertyName);
	...
	protected void autowireByType(
		...
		if (converter == null) {
			logger.info("[SPRING] 自定义日志---【autowireByType】使用的类型转换器是BeanWrapper：" + beanName);
			converter = bw;
		}
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					logger.info("[SPRING] 自定义日志---【autowireByType】resolveDependency：" + beanName);
					...
					logger.info("[SPRING] 自定义日志---【autowireByType】跳过Object类型的属性，因为所有Bean都是Object的子类，无法确定具体注入哪个：" + beanName);
				}
			}
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		for (PropertyValue pv : original) {
			if (pv.isConverted()) {
				...
				logger.info("[SPRING] 自定义日志---【获取Bean】applyPropertyValues步骤：1，调用valueResolver.resolveValueIfNecessary方法："+beanName);
		...
		try {
			deepCopy.forEach( d -> {logger.info("[SPRING] 自定义日志---【获取Bean】"+beanName+",setter注入：" + d.getName());});
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		logger.info("[SPRING] 自定义日志【关键流程-initializeBean】---"+beanName);
		invokeAwareMethods(beanName, bean);

		Object wrappedBean = bean;
		if (mbd == null || !mbd.isSynthetic()) {
			logger.info("[SPRING] 自定义日志---【initializeBean】调用applyBeanPostProcessorsBeforeInitialization方法："+beanName);
			wrappedBean = applyBeanPostProcessorsBeforeInitialization(wrappedBean, beanName);
		}

		try {
			logger.info("[SPRING] 自定义日志---【initializeBean】调用invokeInitMethods方法："+beanName);
			...
			if (initMethodNames != null) {
				for (String initMethodName : initMethodNames) {
					if (StringUtils.hasLength(initMethodName) &&
						...
						logger.info("[SPRING] 自定义日志---invokeInitMethods中，调用invokeCustomInitMethod方法："+beanName);
	...
	protected void invokeCustomInitMethod(String beanName, Object bean, RootBeanDefinition mbd, String initMethodName)
		...
		try {
			...
			logger.info("[SPRING] 自定义日志---invokeCustomInitMethod中，调用methodToInvoke.invoke方法："+beanName);
